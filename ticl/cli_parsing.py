...
def argparser_from_config(parser, description="Train Mothernet"):
    ...
    optimizer.add_argument('--ppo-pack-prior-milestone', type=str,
                           choices=['m4_potential_progress_live'],
                           help='Maintained fit_model PPO pack runner milestone. Older sampled/gated/balanced milestones are analysis-only and must not be launched through fit_model.')
    optimizer.add_argument('--ppo-pack-prior-mode', type=str,
                           choices=['sampled_topology'],
                           help='Prior environment mode used by fit_model training. Maintained training is live sampled_topology only; fixed lists are audit-only and must not be launched through fit_model.')
    optimizer.add_argument('--ppo-pack-fixed-frozen-h-list-csv', type=str,
                           help='Audit-only legacy CSV input. Maintained fit_model training rejects non-empty values.')
    optimizer.add_argument('--ppo-pack-fixed-frozen-h-list-rule', type=str,
                           help='Audit-only legacy fixed-list rule. Maintained fit_model training does not use it.')
    optimizer.add_argument('--ppo-pack-fixed-frozen-h-list-limit', type=int,
                           help='Audit-only legacy fixed-list row limit. Maintained fit_model training does not use it.')
