...
from ticl.analysis import phase3_pair2_high_mass_episode_family_heldout_transfer_compare_pack as compare_mod


def _make_report(*, runtime_suite_name, actor_objective_mode_override, post_train_gap, post_heldout_gap, train_delta, heldout_delta):
    return {
        "config": {
...
def test_build_pair2_high_mass_episode_family_heldout_transfer_compare_pack_detects_material_transfer(
    monkeypatch, tmp_path
):
    locked_train = {
        "audit_entry": "phase3_pair2_high_mass_episode_family_train_update_ab_compare_pack",
        "comparison": {
            "train_full_return_delta_change": 0.12413263320922852,
            "train_suffix_return_delta_change": 0.09682750701904297,
        ...
        "override": {
            "comparison": {
                ...
                "train_full_return_delta": 0.6241326332092285,
                "train_suffix_return_delta": 0.34682750701904297,
            },
            "train_history": [
                {
                    "outer_epoch": 1,
    ...
    override_report = _make_report(
        ...
        post_train_gap={"full_return_gap": 5.4, "suffix_return_gap": 6.5},
        post_heldout_gap={"full_return_gap": 7.3, "suffix_return_gap": 8.15},
        train_delta={"full": 0.6241326332092285, "suffix": 0.34682750701904297},
        heldout_delta={"full": 0.32, "suffix": 0.18},
    )
    ...
    monkeypatch.setattr(compare_mod, "_load_pair2_preflight", lambda: {"preflight_passed": True, "contract_args": {}, "contract_diff": {"allowed_diff": {}, "unexpected_diff": {}}})
    ...
    monkeypatch.setattr(
        ...
        lambda path: {
            ...
            "actor_objective_mode": (
                ...
                else compare_mod.PAIR2_HIGH_MASS_FAMILY_BRANCH_SPECIFIC_MODE
            ),
            "ppo_actor_baseline_mode": "learned",
            "ppo_normalize_advantage": False,
            "ppo_actor_gae_space": "normalized",
    ...
    report = compare_mod.build_pair2_high_mass_episode_family_heldout_transfer_compare_pack()
    ...
    assert report["comparison"]["heldout_full_return_delta_change"] == pytest.approx(0.12)
    assert report["comparison"]["heldout_suffix_return_delta_change"] == pytest.approx(0.08)


def test_build_pair2_high_mass_episode_family_heldout_transfer_compare_pack_rejects_train_side_reproduction_drift(
    monkeypatch, tmp_path
):
    locked_train = {
        "audit_entry": "phase3_pair2_high_mass_episode_family_train_update_ab_compare_pack",
    ...
    with pytest.raises(RuntimeError, match="baseline.post_train_vs_zero.full_return_gap"):
        compare_mod.build_pair2_high_mass_episode_family_heldout_transfer_compare_pack()


def test_build_pair2_high_mass_episode_family_heldout_transfer_compare_pack_rejects_bundle_contract_mismatch(
    monkeypatch, tmp_path
):
    canonical_zero = tmp_path / "zero.json"
    canonical_zero.write_text(
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            "audit_entry": "phase3_pair2_high_mass_episode_family_train_update_ab_compare_pack",
            "comparison": {
                "train_full_return_delta_change": 0.1,
                "train_suffix_return_delta_change": 0.1,
                "heldout_eval_skipped": True,
            },
            "conclusions": {
