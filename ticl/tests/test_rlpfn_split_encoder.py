...
def test_perfeature_tabpfn_split_obs_action_encoder_forward():
    emsize = 32
...
def test_perfeature_tabpfn_forward_policy_step_split_matches_materialized_token():
    torch.manual_seed(20260302)
    emsize = 16
    model = PerFeatureTabPFN(
        n_out=2,
...
def test_rlpfn_model_builder_selects_perfeature_backbone():
    cfg = get_model_default_config("rlpfn")
    cfg["transformer"]["backbone_variant"] = "per_feature_v25"
    cfg["transformer"]["features_per_group"] = 8
    _, model, _, _ = get_model(cfg, device="cpu", should_train=False, verbose=False)
    assert isinstance(model, PerFeatureTabPFN)
    assert model.features_per_group == 8
