...
def _build_policy_model_for_num_features(
    *,
    ...
    model_cls = PerFeatureTabPFN if str(backbone_variant).strip().lower() == "per_feature_v25" else TabPFN
    model = model_cls(
        n_out=1,
...
def _run_family_rollout_with_model(
    *,
    backbone_variant,
    ...
    env_cfg["pg_markov_adjacent_replay_enabled"] = bool(markov_enabled)
    env_cfg["pg_markov_adjacent_replay_sample_prob"] = float(sample_prob)
...
def test_tbptt_one_hop_carry_window_does_not_force_policy_offload_by_default(monkeypatch):
    monkeypatch.delenv("TICL_POLICY_TBPTT_ONEHOP_CARRY_SAVED_TENSORS_OFFLOAD", raising=False)
    assert EnvironmentPrior._tbptt_window_one_hop_policy_offload_needed(
        tbptt_one_hop_boundary_active=True,
        one_hop_boundary_roles={"carry_boundary_out": True},
    ) is False


def test_tbptt_one_hop_carry_window_can_reenable_policy_offload_via_env(monkeypatch):
    monkeypatch.setenv("TICL_POLICY_TBPTT_ONEHOP_CARRY_SAVED_TENSORS_OFFLOAD", "enable")
    assert EnvironmentPrior._tbptt_window_one_hop_policy_offload_needed(
        tbptt_one_hop_boundary_active=True,
        one_hop_boundary_roles={"carry_boundary_out": True, "phase_boundary_carry": True},
    ) is True


def test_tbptt_one_hop_carry_window_markov_only_mode_skips_phase_boundary(monkeypatch):
    monkeypatch.setenv("TICL_POLICY_TBPTT_ONEHOP_CARRY_SAVED_TENSORS_OFFLOAD", "markov_only")
    assert EnvironmentPrior._tbptt_window_one_hop_policy_offload_needed(
        tbptt_one_hop_boundary_active=True,
        one_hop_boundary_roles={
            "carry_boundary_out": True,
            "phase_boundary_carry": True,
            "markov_adjacent_carry": False,
        },
    ) is False
    assert EnvironmentPrior._tbptt_window_one_hop_policy_offload_needed(
        tbptt_one_hop_boundary_active=True,
        one_hop_boundary_roles={
            "carry_boundary_out": True,
            "phase_boundary_carry": False,
            "markov_adjacent_carry": True,
        },
    ) is True
...
def test_alpha_grad_reentrant_rollout_checkpoint_request_falls_back_to_nonreentrant():
    loss_reentrant, stats_reentrant, grads_reentrant = _run_chunk(
        policy_rollout_checkpoint=True,
        policy_rollout_checkpoint_reentrant=True,
        kv_cache_mode="immutable",
        allow_grad_mutable_cache=False,
    ...
    loss_nonreentrant, stats_nonreentrant, grads_nonreentrant = _run_chunk(
        policy_rollout_checkpoint=True,
        policy_rollout_checkpoint_reentrant=False,
        kv_cache_mode="immutable",
        allow_grad_mutable_cache=False,
    ...
    assert torch.allclose(loss_reentrant, loss_nonreentrant, atol=1e-6, rtol=1e-5)
    assert torch.allclose(
        stats_reentrant["objective"],
        stats_nonreentrant["objective"],
        atol=1e-6,
        rtol=1e-5,
    )
    assert len(grads_reentrant) == len(grads_nonreentrant)
    for g_reentrant, g_nonreentrant in zip(grads_reentrant, grads_nonreentrant):
        assert torch.allclose(g_reentrant, g_nonreentrant, atol=1e-6, rtol=1e-5)


def test_perfeature_backbone_alpha_grad_reentrant_rollout_checkpoint_with_recompute_attn_has_finite_grads():
    loss, stats, grads = _run_chunk(
        policy_rollout_checkpoint=True,
        policy_rollout_checkpoint_reentrant=True,
        kv_cache_mode="immutable",
        allow_grad_mutable_cache=False,
...
def test_perfeature_backbone_alpha_grad_tbptt_family_vectorized_has_finite_grads():
    env_cfg = dict(_fixed_env_cfg())
    env_cfg["batch_parallel_backend"] = "torch_vectorized"
    env_cfg["batch_vectorized_grouping"] = "family"
...
def test_alpha_grad_recompute_attn_tbptt_family_vectorized_has_finite_grads():
    env_cfg = dict(_fixed_env_cfg())
    env_cfg["batch_parallel_backend"] = "torch_vectorized"
    env_cfg["batch_vectorized_grouping"] = "family"
...
def test_perfeature_backbone_alpha_grad_recompute_attn_tbptt_family_vectorized_has_finite_grads():
    env_cfg = dict(_fixed_env_cfg())
    env_cfg["batch_parallel_backend"] = "torch_vectorized"
    env_cfg["batch_vectorized_grouping"] = "family"
...
def test_perfeature_backbone_reinforce_family_markov_replay_reports_stats():
    loss, stats, grads = _run_family_rollout_with_model(
        backbone_variant="per_feature_v25",
    ...
    assert int(stats["pg_markov_adjacent_bridge_sampled_count"]) >= 1
    assert len(grads) > 0
    assert all(torch.isfinite(g).all() for g in grads)
