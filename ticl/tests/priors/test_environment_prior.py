...
def test_environment_prior_alpha_grad_single_full_batch_group_matches_tuple_roots():
    prior = EnvironmentPrior({"discount": 1.0})
    action_mean_dense = torch.tensor(
        [
            [[0.2, -0.3], [0.4, 0.1], [-0.2, 0.5]],
    ...
    tuple_roots = (
        action_mean_dense[0].clone().requires_grad_(),
        action_mean_dense[1].clone().requires_grad_(),
    )
    full_group_roots = (
        action_mean_dense[0].clone().requires_grad_(),
        action_mean_dense[1].clone().requires_grad_(),
    ...
    grouped_roots = (
        {
            "indices": (0, 1, 2),
            "action_mean_roots": full_group_roots,
            "action_mask": action_mask,
        },
    )

    tuple_action_mean = torch.stack(tuple_roots, dim=0)
    grouped_action_mean = torch.stack(full_group_roots, dim=0)
    rewards_tuple = (1.5 * tuple_action_mean[..., 0]) - (0.25 * tuple_action_mean[..., 1])
    log_probs_tuple = (0.7 * tuple_action_mean[..., 0]) + (0.2 * tuple_action_mean[..., 1])
    rewards_grouped = (1.5 * grouped_action_mean[..., 0]) - (0.25 * grouped_action_mean[..., 1])
    log_probs_grouped = (0.7 * grouped_action_mean[..., 0]) + (0.2 * grouped_action_mean[..., 1])

    loss_tuple, stats_tuple = prior.alpha_grad_loss_from_rollout_tensors(
        rewards=rewards_tuple,
        log_probs=log_probs_tuple,
        action_mean=tuple_roots,
        action_mask=action_mask,
        discount=1.0,
        variance_eps=1e-6,
    )
    grads_tuple = torch.autograd.grad(loss_tuple, tuple_roots, retain_graph=True)

    loss_grouped, stats_grouped = prior.alpha_grad_loss_from_rollout_tensors(
        rewards=rewards_grouped,
        log_probs=log_probs_grouped,
        action_mean=grouped_roots,
        action_mask=action_mask,
        discount=1.0,
        variance_eps=1e-6,
    )
    grads_grouped = torch.autograd.grad(loss_grouped, full_group_roots, retain_graph=True)

    assert int(stats_tuple["alpha_grad_local_coordinate_enabled"]) == 1
    assert int(stats_grouped["alpha_grad_local_coordinate_enabled"]) == 1
    assert float(stats_grouped["alpha_grad_alpha_mean"]) == pytest.approx(float(stats_tuple["alpha_grad_alpha_mean"]))
    assert float(stats_grouped["alpha_grad_v0_mean"]) == pytest.approx(float(stats_tuple["alpha_grad_v0_mean"]))
    assert float(stats_grouped["alpha_grad_v1_mean"]) == pytest.approx(float(stats_tuple["alpha_grad_v1_mean"]))
    assert torch.allclose(grads_grouped[0], grads_tuple[0], atol=1e-6, rtol=1e-5)
    assert torch.allclose(grads_grouped[1], grads_tuple[1], atol=1e-6, rtol=1e-5)
...
def test_environment_prior_alpha_grad_tbptt_window_payload_keeps_roots_only():
    _seed_everything(2043)
    config = get_prior_config()
    env_cfg = dict(config["prior"]["environment"])
    env_cfg["family"] = {"distribution": "meta_choice", "choice_values": ["scm"]}
    env_cfg["batch_parallel_backend"] = "python_thread"
    ...
    h["action_noise_train_std"] = 0.2
    h["action_noise_eval_std"] = 0.15
    env = prior._sample_environment(h=h, device="cpu")
    prior._rollout_single(
        env=env,
        n_samples=8,
        num_features=24,
        single_eval_pos=3,
        ...
        tbptt_window=4,
        tbptt_reward_sink=payloads.append,
        tbptt_reward_sink_supports_aux=True,
        policy_objective_kind="alpha_grad",
        _policy_collect_action_trace=True,
    )

    assert payloads
    for rewards_window, aux in payloads:
        assert torch.is_tensor(rewards_window)
        assert isinstance(aux, dict)
        policy_trace = aux.get("policy_trace", None)
        assert isinstance(policy_trace, dict)
        assert policy_trace["action_mean"] is None
        assert isinstance(policy_trace.get("action_mean_roots", None), tuple)
        assert policy_trace.get("group_traces", None) is None
        assert torch.is_tensor(policy_trace.get("action_mask", None))


def test_environment_prior_alpha_grad_tbptt_preeval_window_skips_unused_policy_trace_payload():
    _seed_everything(20435)
    config = get_prior_config()
    env_cfg = dict(config["prior"]["environment"])
    env_cfg["family"] = {"distribution": "meta_choice", "choice_values": ["scm"]}
    env_cfg["batch_parallel_backend"] = "python_thread"
    ...
    first_rewards, first_aux = payloads[0]
    second_rewards, second_aux = payloads[1]
    assert torch.is_tensor(first_rewards)
    assert torch.is_tensor(second_rewards)
    assert isinstance(first_aux, dict)
    assert isinstance(second_aux, dict)
    assert "policy_trace" not in first_aux
    assert "reinforce" not in first_aux
    assert isinstance(second_aux.get("policy_trace", None), dict)
    assert isinstance(second_aux.get("reinforce", None), dict)


def test_environment_prior_alpha_grad_family_tbptt_outer_merge_keeps_group_traces_only():
    _seed_everything(2044)
    config = get_prior_config()
    env_cfg = dict(config["prior"]["environment"])
    env_cfg["family"] = {"distribution": "meta_choice", "choice_values": ["scm"]}
    env_cfg["batch_parallel_backend"] = "torch_vectorized"
    ...
    assert payloads
    saw_eval_payload = False
    for rewards_window, aux in payloads:
        assert torch.is_tensor(rewards_window)
        assert isinstance(aux, dict)
        policy_trace = aux.get("policy_trace", None)
        if policy_trace is None:
            assert "reinforce" not in aux
            continue
        saw_eval_payload = True
        assert isinstance(policy_trace, dict)
        assert policy_trace["action_mean"] is None
        assert policy_trace.get("action_mean_roots", None) is None
        group_traces = policy_trace.get("group_traces", None)
        assert isinstance(group_traces, tuple)
        assert len(group_traces) > 0
        assert torch.is_tensor(policy_trace.get("action_mask", None))
    assert saw_eval_payload


def test_environment_prior_alpha_grad_family_tbptt_outer_merge_tolerates_preeval_windows_without_policy_trace():
    _seed_everything(20445)
    config = get_prior_config()
    env_cfg = dict(config["prior"]["environment"])
    env_cfg["family"] = {"distribution": "meta_choice", "choice_values": ["scm"]}
    env_cfg["batch_parallel_backend"] = "torch_vectorized"
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA required for reference SCM hidden-loop checkpoint check")
def test_environment_prior_strict_reference_scm_hidden_loop_checkpoint_preserves_generator_and_grad_semantics(monkeypatch):
    ...
    x_base = torch.linspace(
        -0.55,
        0.75,
        steps=len(h_list) * in_cap,
        dtype=torch.float32,
        device="cuda",
    ).reshape(len(h_list), in_cap)

    def _build(checkpoint_flag: str):
        _seed_everything(20260319)
        monkeypatch.setenv("TICL_POLICY_REFERENCE_SCM_HIDDEN_AFFINE_FUSED", "1")
        monkeypatch.setenv("TICL_POLICY_REFERENCE_SCM_HIDDEN_UPDATE_FUSED", "1")
        monkeypatch.setenv("TICL_POLICY_REFERENCE_SCM_HIDDEN_UPDATE_SAMPLE_FUSED", "1")
        monkeypatch.setenv("TICL_POLICY_REFERENCE_SCM_HIDDEN_UPDATE_GRAD_FUSED", "1")
        monkeypatch.setenv("TICL_POLICY_REFERENCE_SCM_HIDDEN_LOOP_CHECKPOINT", checkpoint_flag)
        prior = EnvironmentPrior({})
        return prior._build_reference_scm_joint_transition_padded_batch_fn(
            in_dims,
            state_dims,
    ...
    x_off = x_base.clone().requires_grad_(True)
    gens_off = _noise_generators()
    state_off, reward_off = fn_off(x_off, generators_for_noise=gens_off)
    loss_off = state_off.sum() + reward_off.sum()
    grad_off = torch.autograd.grad(loss_off, x_off)[0]
    gen_state_off = [g.get_state().cpu() for g in gens_off]

    x_on = x_base.clone().requires_grad_(True)
    gens_on = _noise_generators()
    state_on, reward_on = fn_on(x_on, generators_for_noise=gens_on)
    loss_on = state_on.sum() + reward_on.sum()
    grad_on = torch.autograd.grad(loss_on, x_on)[0]
    gen_state_on = [g.get_state().cpu() for g in gens_on]

    assert torch.allclose(state_off, state_on, atol=1e-6, rtol=1e-6)
    assert torch.allclose(reward_off, reward_on, atol=1e-6, rtol=1e-6)
    assert torch.allclose(grad_off, grad_on, atol=2e-5, rtol=2e-5)
    assert len(gen_state_off) == len(gen_state_on)
    for state_ref, state_ckpt in zip(gen_state_off, gen_state_on):
        assert torch.equal(state_ref, state_ckpt)
...
def test_environment_prior_alpha_grad_family_tbptt_markov_adjacent_future_replay_changes_gradients():
    config = get_prior_config()
    env_cfg = dict(config["prior"]["environment"])
    env_cfg["family"] = {"distribution": "meta_choice", "choice_values": ["scm"]}
    env_cfg["batch_parallel_backend"] = "torch_vectorized"
    ...
    def _run_case(markov_enabled, sample_prob):
        _seed_everything(2084)
        prior = EnvironmentPrior(
            dict(
                env_cfg,
                pg_markov_adjacent_replay_enabled=bool(markov_enabled),
                pg_markov_adjacent_replay_sample_prob=float(sample_prob),
            )
        )
        ...
        loss.backward()
        return loss.detach(), stats, policy.net.weight.grad.detach().clone()

    loss_no_markov, stats_no_markov, grad_no_markov = _run_case(False, 0.0)
    loss_with_markov, stats_with_markov, grad_with_markov = _run_case(True, 1.0)

    assert torch.isfinite(loss_no_markov)
    assert torch.isfinite(loss_with_markov)
    assert int(stats_no_markov["alpha_grad_enabled"]) == 1
    assert int(stats_with_markov["alpha_grad_enabled"]) == 1
    assert int(stats_with_markov["pg_markov_adjacent_replay_enabled"]) == 1
    assert float(stats_with_markov["pg_markov_adjacent_replay_sample_prob"]) == 1.0
    assert int(stats_no_markov["pg_bridge_replay_count"]) == 1
    assert int(stats_with_markov["pg_markov_adjacent_bridge_sampled_count"]) == 2
    assert int(stats_with_markov["pg_bridge_replay_count"]) == 3
    assert not torch.equal(grad_no_markov, grad_with_markov)
    assert float((grad_with_markov - grad_no_markov).norm()) > 0.0
