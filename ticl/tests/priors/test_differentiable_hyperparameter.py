...
def test_differentiable_hyper_uniform():
    ...
    assert samples[0] == pytest.approx(0.3745401188473625)
    assert samples[10] == pytest.approx(0.020584494295802447)
...
def test_differentiable_hyper_meta_beta():
    ...
    samples = [hyperparameter() for _ in range(11)]
    assert samples[0]() == pytest.approx(0.15255399122616758)
    assert samples[0]() == pytest.approx(0.2707540740311954)
    assert samples[10]() == pytest.approx(0.17798037863404909)
    assert samples[10]() == pytest.approx(0.47444729678154285)
...
def test_differentiable_hyper_meta_trunc_norm_log_scaled():
    ...
    samples = [hyperparameter() for _ in range(11)]
    assert samples[0]() == pytest.approx(0.0014598013986488975)
    assert samples[0]() == pytest.approx(0.001735194550030197)
    assert samples[10]() == pytest.approx(0.013383656409053687)
    assert samples[10]() == pytest.approx(0.013613155757117055)
...
def test_differentiable_hyper_meta_meta_choice_mixed():
    ...
    samples = [hyperparameter() for _ in range(11)]
    draws = [samples[0]()(), samples[0]()(), samples[10]()(), samples[10]()()]
    assert all(d in {"a", "b", "c"} for d in draws)
