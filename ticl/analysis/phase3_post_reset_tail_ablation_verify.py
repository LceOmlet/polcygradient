...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Phase 3 verify whether removing post-reset terminal-tail mass reduces heldout bias (no rerun)."
    )
    parser.add_argument(
    ...
    for row in payload["env_rows"]:
        post_reset_tail_removed = row["tail_len_rows"][tail_len_key]["post_reset_terminal_tail"]["removed_positive_mass"]
        base_items.append(
            {
                ...
                "post_reset_tail_remaining_mass": float(row["base_positive_mass"]) - float(post_reset_tail_removed),
            }
        )

    base_summary = _summary(base_items, mass_key="base_positive_mass")
    ablated_summary = _summary(base_items, mass_key="post_reset_tail_remaining_mass")

    result = {
        "audit_entry": "phase3_post_reset_tail_ablation_verify",
        "config": {
            "tail_mask_probe_json": str(probe_path),
            "tail_len": int(args.tail_len),
        },
        "base_summary": base_summary,
        "post_reset_tail_ablation_summary": ablated_summary,
        "deltas": {
        ...
        "conclusions": {
            "post_reset_tail_ablation_reduces_reset_bias": bool(
                ablated_summary["corr_vs_terminal_resets"] < base_summary["corr_vs_terminal_resets"]
            ),
            "post_reset_tail_ablation_improves_pre_gap_alignment": bool(
