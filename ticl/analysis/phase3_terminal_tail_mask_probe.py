...
def _tail_masks(valid_len: int, segments: list[tuple[int, int]], tail_len: int) -> dict[str, np.ndarray]:
    ...
    first_only = np.zeros((valid_len,), dtype=bool)
    post_only = np.zeros((valid_len,), dtype=bool)
    if not segments:
        return {"terminal_tail": all_tail, "first_terminal_tail": first_tail, "post_reset_terminal_tail": post_tail}
    for idx, (start, end) in enumerate(segments):
        if idx == 0:
            first_only[start:end] = True
        else:
            post_only[start:end] = True
    ...
    return {
        ...
        "first_episode_only": first_only,
        "post_reset_only": post_only,
    }
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Phase 3 terminal-tail mask decomposition under the lightweight heldout contract."
    )
    ...
    for env_idx in range(int(objective_masks.shape[1])):
        ...
        for tail_len in TAIL_LENS:
            ...
            for key, mask in masks.items():
                keep_mask = ~mask
                tail_metrics[key] = {
                    "removed_positive_mass": float(np.clip(env_adv[mask], a_min=0.0, a_max=None).sum()),
                    "remaining_positive_mass": _mass_after_ablation(env_adv, keep_mask),
                }
    ...
    for tail_len in TAIL_LENS:
        ...
        for variant in MASK_RULES:
            ...
            per_variant[variant] = _summary(items, mass_key="positive_mass")
