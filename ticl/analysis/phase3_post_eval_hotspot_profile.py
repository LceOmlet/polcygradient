...
@contextmanager
def _timed_method(obj, method_name: str, bucket: dict[str, dict[str, float]]):
    original = getattr(obj, method_name)
    bucket.setdefault(method_name, {"calls": 0, "total_s": 0.0})
    ...
    def _wrapped(*args, **kwargs):
        ...
        try:
            ...
            rec = bucket[method_name]
            rec["calls"] += 1
            rec["total_s"] += float(time.perf_counter() - t0)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Single-env hotspot profile for Phase 3 post-train/post-heldout serial eval."
    )
    ...
    t0 = time.perf_counter()
    with _timed_method(algo.policy, "_build_obs_from_rollout_step_inputs", timers):
        with _timed_method(eval_prior, "_apply_state_full_rms", timers):
            with _timed_method(eval_prior, "_apply_terminal_reset_step", timers):
                with _timed_method(eval_prior, "_terminal_tail_event_from_signal", timers):
    ...
    report = {
        "audit_entry": "phase3_post_eval_hotspot_profile",
        "phase2_preflight": {
            "required": True,
