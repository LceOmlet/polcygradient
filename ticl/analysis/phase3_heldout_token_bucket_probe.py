...
from ticl.analysis.phase3_train_token_bucket_probe import (
    BUCKET_NAMES,
    _bucket_metrics,
    _mean,
)
...
def _load_heldout_subset_deltas(path: str) -> dict[int, dict[str, float]]:
    payload = json.loads(Path(path).expanduser().resolve().read_text())
    out: dict[int, dict[str, float]] = {}
    for item in payload["heldout_subset"]["combined"]["env_items"]:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Phase 3 lightweight heldout objective token-bucket probe under the same many-env contract."
    )
    ...
    parser.add_argument("--reuse-heldout-subset-delta-json", type=str, required=True)
    ...
    subset_deltas = _load_heldout_subset_deltas(str(args.reuse_heldout_subset_delta_json))
    ...
    subset_mass = [
        float(sum(float(bucket["positive_mass"]) for bucket in row["bucket_metrics"].values()))
    ...
    for bucket_name in BUCKET_NAMES:
        contrast[bucket_name] = {
            "positive_mass_share": _mean(
                low_mass_positive,
                lambda row: row["bucket_metrics"][bucket_name]["positive_mass_share"],
            )
            - _mean(
                high_mass_nonpositive,
                lambda row: row["bucket_metrics"][bucket_name]["positive_mass_share"],
            ),
            "negative_mass_share": _mean(
                low_mass_positive,
                lambda row: row["bucket_metrics"][bucket_name]["negative_mass_share"],
            )
            - _mean(
                high_mass_nonpositive,
                lambda row: row["bucket_metrics"][bucket_name]["negative_mass_share"],
            ...
            "value_corr": _mean(
                low_mass_positive,
                lambda row: row["bucket_metrics"][bucket_name]["raw_value_return_corr"],
            )
            - _mean(
                high_mass_nonpositive,
                lambda row: row["bucket_metrics"][bucket_name]["raw_value_return_corr"],
    ...
    q12_negative = (
        float(bucket_alignment["q1"]["corr_delta_vs_positive_mass_share"]) < 0.0
        and float(bucket_alignment["q2"]["corr_delta_vs_positive_mass_share"]) < 0.0
    )
    q34_positive = (
        float(bucket_alignment["q3"]["corr_delta_vs_positive_mass_share"]) > 0.0
        and float(bucket_alignment["q4"]["corr_delta_vs_positive_mass_share"]) > 0.0
    )

    report = {
        "audit_entry": "phase3_heldout_token_bucket_probe",
        "phase2_preflight": {
            "required": True,
        ...
        "config": {
            ...
            "reuse_heldout_subset_delta_json": str(Path(args.reuse_heldout_subset_delta_json).expanduser().resolve()),
            "n_samples": int(args.n_samples),
            "single_eval_pos": int(args.single_eval_pos),
        ...
        "conclusions": {
            ...
            "heldout_q12_anti_correlated_with_delta": bool(q12_negative),
            "heldout_q34_positively_correlated_with_delta": bool(q34_positive),
            "heldout_q3_aligns_stronger_than_q4": bool(
                float(bucket_alignment["q3"]["corr_delta_vs_positive_mass_share"])
                > float(bucket_alignment["q4"]["corr_delta_vs_positive_mass_share"])
            ),
            "heldout_q34_success_pattern_matches_train": bool(q12_negative and q34_positive),
            "heldout_low_mass_positive_has_better_q1_value_corr": bool(
                float(contrast["q1"]["value_corr"]) > 0.0
            ),
        },
    }
