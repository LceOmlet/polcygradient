...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Phase 3 verify whether post-reset-only ablation removes heldout bias (no rerun)."
    )
    parser.add_argument(
        ...
        base_items.append(
            {
                ...
                "post_reset_remaining_mass": row["tail_len_rows"][tail_len_key]["post_reset_only"][
    ...
    ablated_summary = _summary(base_items, mass_key="post_reset_remaining_mass")

    result = {
        "audit_entry": "phase3_post_reset_ablation_verify",
        "config": {
            "tail_mask_probe_json": str(probe_path),
            "tail_len": int(args.tail_len),
        },
        "base_summary": base_summary,
        "post_reset_ablation_summary": ablated_summary,
        "deltas": {
        ...
        "conclusions": {
            "post_reset_ablation_reduces_reset_bias": bool(
                ablated_summary["corr_vs_terminal_resets"] < base_summary["corr_vs_terminal_resets"]
            ),
            "post_reset_ablation_improves_pre_gap_alignment": bool(
