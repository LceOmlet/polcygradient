...
def _inject_seed_mode(algo, *, env_seed: int | None, rollout_seed: int | None):
...
def run_seed_source_ablation(
    *,
    ...
    def _collect_series(*, env_seed_override: int | None, rollout_seed_override: int | None):
        algo, callback, vec_env, prior = _build()
        original = _inject_seed_mode(
            algo,
            env_seed=env_seed_override,
            rollout_seed=rollout_seed_override,
        )
        try:
            collected = []
    ...
    return {
        "audit_entry": "critic_rollout_seed_source_ablation",
        ...
        "conditions": {
            "none": _collect_series(env_seed_override=None, rollout_seed_override=None),
            "env_only": _collect_series(env_seed_override=int(train_env_seed), rollout_seed_override=None),
            "rollout_only": _collect_series(env_seed_override=None, rollout_seed_override=int(rollout_seed)),
            "both": _collect_series(env_seed_override=int(train_env_seed), rollout_seed_override=int(rollout_seed)),
        },
    }
...
def main():
    ...
    parser.add_argument(
        "--output-json",
        type=str,
        default="/home/chen/RLPFN/artifacts/critic_rollout_seed_source_ablation.json",
    )
    args = parser.parse_args()

    report = run_seed_source_ablation(
