...
def _tail_masks(valid_len: int, segments: list[tuple[int, int]], tail_len: int) -> dict[str, np.ndarray]:
    first_tail = np.zeros((valid_len,), dtype=bool)
    post_tail = np.zeros((valid_len,), dtype=bool)
    all_tail = np.zeros((valid_len,), dtype=bool)
    if not segments:
        return {"terminal_tail": all_tail, "first_terminal_tail": first_tail, "post_reset_terminal_tail": post_tail}
    for idx, (start, end) in enumerate(segments):
        tail_start = max(int(start), int(end) - int(tail_len))
        mask = np.zeros((valid_len,), dtype=bool)
        mask[tail_start:end] = True
        all_tail |= mask
        if idx == 0:
            first_tail |= mask
        else:
            post_tail |= mask
    return {
        "terminal_tail": all_tail,
        "first_terminal_tail": first_tail,
        "post_reset_terminal_tail": post_tail,
    }


def _mass_after_ablation(values: np.ndarray, keep_mask: np.ndarray) -> float:
    if int(keep_mask.sum()) <= 0:
        return 0.0
    kept = values[keep_mask]
    return float(np.clip(kept, a_min=0.0, a_max=None).sum())


def main() -> int:
    t0 = time.perf_counter()
    parser = argparse.ArgumentParser(
        description="Phase 3 tail-bias propagation probe: raw return vs GAE-adv on same heldout rollout."
    )
    ...
    raw_returns = rollout_buffer._get_flat_raw_returns_tensor(dtype=torch.float32).detach().cpu().numpy().reshape(
        objective_masks.shape
    )
    vec_env.close()
    ...
    for env_idx in range(int(objective_masks.shape[1])):
        ...
        row = {
            ...
            "base_return_positive_mass": float(np.clip(env_returns, a_min=0.0, a_max=None).sum()),
            "base_adv_positive_mass": float(np.clip(env_adv, a_min=0.0, a_max=None).sum()),
            "tail_len_rows": {},
        }
        for tail_len in TAIL_LENS:
            masks = _tail_masks(int(obj.sum()), segments, int(tail_len))
            tail_metrics = {}
            for key, mask in masks.items():
                keep_mask = ~mask
                tail_metrics[key] = {
                    "return_removed_positive_mass": float(np.clip(env_returns[mask], a_min=0.0, a_max=None).sum()),
                    "return_remaining_positive_mass": _mass_after_ablation(env_returns, keep_mask),
                    "adv_removed_positive_mass": float(np.clip(env_adv[mask], a_min=0.0, a_max=None).sum()),
                    "adv_remaining_positive_mass": _mass_after_ablation(env_adv, keep_mask),
                }
            row["tail_len_rows"][str(tail_len)] = tail_metrics
        env_rows.append(row)
    ...
    for tail_len in TAIL_LENS:
        ...
        per_variant = {}
        for variant in ("terminal_tail", "first_terminal_tail", "post_reset_terminal_tail"):
            return_items = [
                {
                    ...
                    "positive_mass": row["tail_len_rows"][tail_len_key][variant]["return_remaining_positive_mass"],
                }
                for row in env_rows
            ]
            adv_items = [
                {
                    ...
                    "pre_vs_zero_suffix_gap": row["pre_vs_zero_suffix_gap"],
                    "raw_value_return_corr": row["raw_value_return_corr"],
                    "positive_mass": row["tail_len_rows"][tail_len_key][variant]["adv_remaining_positive_mass"],
                }
                for row in env_rows
            ]
            per_variant[variant] = {
                "raw_return_positive_mass": _summary(return_items, mass_key="positive_mass"),
                "gae_adv_positive_mass": _summary(adv_items, mass_key="positive_mass"),
            }
        summaries[tail_len_key] = per_variant

    result = {
        "audit_entry": "phase3_tail_bias_propagation_probe",
        "config": {
