...
from ticl.analysis.phase3_pair2_high_mass_episode_family_train_update_ab_compare_pack import (
...
PAIR2_HIGH_MASS_FAMILY_LOCKED_TRAIN_COMPARE_JSON = (
    "/home/chen/RLPFN/artifacts/phase3_pair2_high_mass_episode_family_train_update_ab_compare_pack.json"
)
PAIR2_HIGH_MASS_FAMILY_HELDOUT_TRANSFER_BASELINE_RESUME_EVAL_JSON = (
    "/home/chen/RLPFN/artifacts/phase3_pair2_high_mass_episode_family_heldout_transfer_baseline_resume_eval.json"
)
PAIR2_HIGH_MASS_FAMILY_HELDOUT_TRANSFER_OVERRIDE_RESUME_EVAL_JSON = (
...
def _load_locked_train_compare() -> dict[str, Any]:
    path = Path(PAIR2_HIGH_MASS_FAMILY_LOCKED_TRAIN_COMPARE_JSON).expanduser().resolve()
    payload = json.loads(path.read_text(encoding="utf-8"))
    if (
        str(payload.get("audit_entry", ""))
        != "phase3_pair2_high_mass_episode_family_train_update_ab_compare_pack"
    ):
        raise RuntimeError(f"Expected locked high-mass-family train compare artifact at {path}")
    conclusions = dict(payload.get("conclusions", {}))
    if not bool(conclusions.get("target_side_compare_is_efficacy_eligible", False)):
        raise RuntimeError(f"Locked train compare is not efficacy-eligible: {path}")
    if not bool(conclusions.get("target_side_train_update_changes_trajectory", False)):
        raise RuntimeError(f"Locked train compare does not show train-side trajectory change: {path}")
    if bool(payload.get("comparison", {}).get("heldout_eval_skipped", False)) is not True:
        raise RuntimeError(
            "Locked high-mass-family train compare should remain train-side only; "
...
def _assert_bundle_contract_matches_expected(
    ...
    for key, expected in required.items():
        ...
        if observed != expected:
            raise RuntimeError(
                f"Pair2 high-mass-family {arm_name} bundle contract mismatch for {key}: "
...
def _run_resumed_eval_audit(
    *,
    ...
    mode_label = "baseline_resume" if actor_objective_mode_override is None else str(actor_objective_mode_override)
    print(f"[pair2-high-mass-family-heldout] start audit mode={mode_label}", flush=True)
    ...
    print(
        f"[pair2-high-mass-family-heldout] done audit mode={mode_label}; wrote {resolved_path}",
        flush=True,
    )
    return report


def build_pair2_high_mass_episode_family_heldout_transfer_compare_pack() -> dict[str, Any]:
    ...
    _assert_bundle_contract_matches_expected(
        ...
        expected_actor_objective_mode=PAIR2_HIGH_MASS_FAMILY_BRANCH_SPECIFIC_MODE,
        arm_name="override",
    )
    ...
    print("[pair2-high-mass-family-heldout] start baseline resumed eval", flush=True)
    baseline = _run_resumed_eval_audit(
        actor_objective_mode_override=None,
        resume_post_policy_bundle_path=PAIR2_HIGH_MASS_FAMILY_BASELINE_POST_POLICY_BUNDLE_PATH,
        reuse_zero_control_json=None,
        reuse_pre_policy_json=None,
        output_json_path=PAIR2_HIGH_MASS_FAMILY_HELDOUT_TRANSFER_BASELINE_RESUME_EVAL_JSON,
    )

    print("[pair2-high-mass-family-heldout] start override resumed eval", flush=True)
    with tempfile.TemporaryDirectory(prefix="pair2_high_mass_family_heldout_") as tmpdir:
        ...
        override = _run_resumed_eval_audit(
            ...
            resume_post_policy_bundle_path=PAIR2_HIGH_MASS_FAMILY_OVERRIDE_POST_POLICY_BUNDLE_PATH,
            reuse_zero_control_json=reuse_zero_control_json,
            reuse_pre_policy_json=reuse_pre_policy_json,
            output_json_path=PAIR2_HIGH_MASS_FAMILY_HELDOUT_TRANSFER_OVERRIDE_RESUME_EVAL_JSON,
        )
    ...
    if _shared_contract_context(baseline_ctx) != _shared_contract_context(override_ctx):
        raise ValueError(
            "Baseline and override high-mass-family heldout-transfer audits must share the same suite contract, "
    ...
    return {
        ...
        "locked_train_side_source": {
            "source_path": PAIR2_HIGH_MASS_FAMILY_LOCKED_TRAIN_COMPARE_JSON,
            "train_suffix_return_delta_change": float(
        ...
        "bundle_contracts": {
            "baseline_bundle_path": PAIR2_HIGH_MASS_FAMILY_BASELINE_POST_POLICY_BUNDLE_PATH,
            "override_bundle_path": PAIR2_HIGH_MASS_FAMILY_OVERRIDE_POST_POLICY_BUNDLE_PATH,
        ...
        "baseline_eval_report": {
            "path": PAIR2_HIGH_MASS_FAMILY_HELDOUT_TRANSFER_BASELINE_RESUME_EVAL_JSON,
            "comparison": baseline_comparison,
            "train_history": list(baseline["train_history"]),
        },
        "override_eval_report": {
            "path": PAIR2_HIGH_MASS_FAMILY_HELDOUT_TRANSFER_OVERRIDE_RESUME_EVAL_JSON,
        ...
        "recommendation": {
            ...
            "shared_core_unchanged": True,
            "reason": (
                "Read heldout transfer only after reproducing the locked train-side effect from the saved "
                "post-policy bundles under the same guarded contract."
            ),
        },
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Pair2 heldout-transfer compare pack for the locked high-mass branch-specific episode family."
    )
    parser.add_argument(
        "--output-json",
        type=str,
        default="/home/chen/RLPFN/artifacts/phase3_pair2_high_mass_episode_family_heldout_transfer_compare_pack.json",
    )
    ...
    report = build_pair2_high_mass_episode_family_heldout_transfer_compare_pack()
    output_path.parent.mkdir(parents=True, exist_ok=True)
