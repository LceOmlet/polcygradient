...
def _build_suite_algo(
    *,
    ...
    algo, callback, vec_env = build_recurrent_ppo(
        ...
        target_kl=float(target_kl),
        strict_fixed_env_mode=True,
        env_rng_seeds=list(suite["env_seeds"]),
        rollout_rng_seeds=list(suite["rollout_seeds"]),
        deterministic_actor_sampling=True,
        deterministic_batch_plan=True,
        strict_native_rollout=False,
        restore_validation_policy_state=False,
...
def _collect_rollout(algo, callback, vec_env, *, progress_timestep: int, total_timesteps: int) -> None:
    ...
    if not ok:
        raise RuntimeError("collect_rollouts returned False during scalar value-head probe.")
            ...
            history.append(
                {
                    ...
                    "heldout_rollout": heldout_quality,
                }
            )
        return {
            "value_head_impl": str(value_head_impl),
...
def run_phase2_scalar_value_head_impl_probe(
    *,
    checkpoint_path: str,
    ...
    return {
        "audit_entry": "phase2_scalar_value_head_impl_probe",
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-json",
        type=str,
        default="/home/chen/RLPFN/artifacts/phase2_scalar_value_head_impl_probe.json",
    )
    args = parser.parse_args()

    report = run_phase2_scalar_value_head_impl_probe(
