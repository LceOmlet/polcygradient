...
@torch.no_grad()
def _forward_pack_attention_internal(policy, bucket_obs: torch.Tensor, *, max_layer: int = 1) -> dict:
    ...
    with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=bool(tokens_work.is_cuda)):
        for layer_idx, block in enumerate(policy.rlpfn_model.rwkv_core.blocks):
            ...
            stages[f"layer{layer_idx}.r"] = r.transpose(0, 1)
            stages[f"layer{layer_idx}.k_raw"] = k_raw.transpose(0, 1)
            stages[f"layer{layer_idx}.v"] = v.transpose(0, 1)
            stages[f"layer{layer_idx}.g"] = g.transpose(0, 1)
            stages[f"layer{layer_idx}.kk_norm"] = kk_norm.transpose(0, 1)
...
@torch.no_grad()
def _extract_attention_internal_dataset(algo, *, max_layer: int = 1) -> dict:
    ...
    for rollout_data in rollout_buffer.get_gpu_flat(algo.batch_size):
        ...
        while cursor < n_seq:
            ...
            stage_pack = _forward_pack_attention_internal(algo.policy, bucket_obs, max_layer=int(max_layer))
...
def run_phase2_attention_internal_geometry_probe(
    *,
    checkpoint_path: str,
    ...
    try:
        ...
        algo.train()
        same_dataset = _extract_attention_internal_dataset(algo, max_layer=int(max_layer))
        ...
        next_dataset = _extract_attention_internal_dataset(algo, max_layer=int(max_layer))
    finally:
        vec_env.close()

    results = {}
    for stage_name in list(same_dataset["stages"].keys()):
...
def main():
    parser = argparse.ArgumentParser(description="Attention-internal geometry probe for Phase 2 critic path.")
    parser.add_argument("checkpoint_path", type=str)
    parser.add_argument("--device", type=str, default=None)
    parser.add_argument("--output-json", type=str, required=True)
    args = parser.parse_args()

    result = run_phase2_attention_internal_geometry_probe(
