...
def _late_dist2_tail_mass(adv_values: np.ndarray, segments: list[tuple[int, int]]) -> float:
    removed = 0.0
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Phase 3 ablation: remove post-reset tail dist<=2 within late-step region (requires rollout)."
    )
    ...
    for env_idx in range(int(objective_masks.shape[1])):
        ...
        removed_mass = _late_dist2_tail_mass(env_adv, segments)
        row = {
    ...
    result = {
        "audit_entry": "phase3_post_reset_tail_dist2_late_ablation_verify",
        "config": {
        ...
        "post_reset_tail_dist2_late_ablation_summary": ablated_summary,
        "deltas": {
